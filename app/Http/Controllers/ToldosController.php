...
class ToldosController extends Controller
{
    ...
    public function fetchData(Request $request){
        ...
        switch($mechanism_id) {
            case 1:
                $accesories = $handle_total + $total_canopy + $total_bambalina;
                $price = ((((($sprice+$total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories;
                break;
            case 2:
                $accesories = $control_total + $sensor_total + $voice_total + $total_canopy + $total_bambalina;
                $price = ((((($sprice+$total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories;
        ...
        $price_manual = ((((($manual->price+$total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories_manual;
        ...
        $price_somfy = ((((($somfy->price+$total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories_somfy;
        ...
        $price_cmo = ((((($cmo->price+ $total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories_cmo;
        ...
        $price_tube = ((((($tube->price+$total_cover)*1.16) / (1-$utility)) * $quantity) * (1-($user->discount/100))) + $accesories_tube;
